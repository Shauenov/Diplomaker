...
def main():
    ...
    students = parse_excel_sheet(df, '3Ғ-1', start_row=5)
    ...
    for subj, g in s['grades'].items():
        ...
        h = g.get('hours', '')
        c = g.get('credits', '')
        if 'пм' in subj.lower() or 'ро' in subj.lower() or 'км' in subj.lower() or 'он' in subj.lower() or 'бм' in subj.lower():
            print(f"{name[:45]:<45} | H: {str(h):<5} | C: {str(c):<5}")
